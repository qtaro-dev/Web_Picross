...
このファイルは、ポートフォリオ向けに短縮する前のREADME詳細情報を退避したものです。
...
Supabase Authでは正規のメールアドレス + パスワードで登録します。ログイン画面ではユーザー名とパスワードだけを入力し、アプリ側の `/api/resolve-login-email` がサーバー側でログイン補助を行います。このAPIは実メールアドレスをブラウザへ返しません。ユーザー登録、パスワード再設定、確認メール再送では引き続きメールアドレス入力が必要です。
...
初期パズルに残っていた `"1"` / `"2"` などの番号キーと、一部の種別不一致キーは正式な `puzzle_key` へ整理済みです。既存Supabase DBへ反映する場合は、先に [docs/supabase/013_normalize_puzzle_keys.sql](supabase/013_normalize_puzzle_keys.sql) の確認SELECTで対象行を確認し、問題なければ同SQLを手動適用してください。このSQLは `puzzles.id` UUIDを変更せず、`puzzle_key` だけを更新します。
...
- ログインはユーザー名とパスワードのみで行います。メールアドレスは登録、パスワード再設定、確認メール再送時だけ入力します。
- 登録用メールアドレスは50文字以内で、メールアドレス形式のみ受け付けます。
...
- 「ユーザー名を記憶する」をONにしてログインすると、ブラウザ側 `localStorage` にユーザー名だけを保存し、次回ログイン画面で復元します。OFFでログインすると保存済みユーザー名を削除します。
- パスワードは `localStorage` に保存しません。旧バージョンで保存済みのパスワードキーがある場合は、ログイン画面初期化時に削除します。
...
- ローカルサーバーの `/api/user-progress` は、`/api/login` で発行された一時セッショントークンとユーザー名が一致する場合だけ保存します。サーバー再起動後は再ログインが必要です。
...
- 読み込み済みJSONスロット欄は折りたたみ可能で、開閉状態は `localStorage` に保存します。
- エディタ盤面はデータを変更せずに50%〜200%で表示倍率を変更できます。
- 完成プレビューは現在の盤面をリアルタイムに小型キャンバスへ反映します。
...
- 判定では具体的な数値を出さず、正解率や誤塗りの有無に応じた短いメッセージを表示します。
- ゲーム画面には盤面の縮小・拡大ボタンがあります。セルサイズ自体を変更するため、クリックやドラッグ位置はずれません。
- 実プレイ画面は1024×768〜1920×1080を主な想定範囲とし、盤面表示領域を優先するレスポンシブ配置にしています。
- 左操作パネルは折りたたみ可能で、開閉状態は `localStorage` に保存します。
- カラーパレットは使用色のみを左操作パネル内へコンパクト表示し、ゲーム中は色ID文字を表示しません。
- 20×20以上の大盤面では盤面側の表示領域を優先し、盤面コンテナ内スクロールと拡大縮小を併用して操作します。
- 大盤面の盤面コンテナ内スクロール位置は再描画後も可能な範囲で保持します。
- セル入力と行列ハイライトは、盤面全体の再描画ではなく対象セル・対象行列だけを更新します。
- Endlessモードでは、表示切り替えと1倍〜4倍の倍率保存に対応したcanvasミニマップで現在位置確認とクリック移動ができます。
- クリア後にOKでゲーム画面へ戻った場合も、盤面操作はロックしたまま、メニュー・セレクトへの移動は確認なしで行えます。

## お知らせ

- メニューの「お知らせ」から、更新情報を2カラム画面で確認できます。
- Supabase設定済みの場合、お知らせは `news_posts` の公開済み記事を優先して表示します。未設定または取得失敗時は `data/news.json` をフォールバックとして使います。
...
- `news-images` は公開お知らせ画像用のpublic bucketです。公開済み記事に使う画像は安定した公開URLで表示できますが、下書き記事の画像、未使用画像、誤アップロード画像もURLを知っている人は閲覧できます。機密画像はアップロードせず、不要画像は管理者画面のStorage削除で消してください。完全非公開の下書き画像が必要な場合は、private bucket + signed URLなど別設計で扱います。
- 一般お知らせ画像と管理者プレビュー画像では、右クリックメニューとドラッグ操作をUI上で抑止しています。これはcasual copy防止・誤操作防止であり、DevTools、Network、直接URL、スクリーンショット等を防ぐセキュリティ対策ではありません。
- 管理者のお知らせ一覧は10件ごとにページングします。新規作成後は、一般向け表示順に合わせた一覧の1ページ目へ戻ります。
- お知らせ編集では `下書き保存` と `今すぐ公開` を分けています。今すぐ公開は現在日時を公開日時へ設定し、下書き保存は非公開として保存します。
- 本文は任意ですが、本文または画像のどちらかは必要です。アップロード画像は本文下のメイン画像として表示され、本文中へURLを貼る必要はありません。
- 本文はプレーンテキストとして改行を反映します。
- Supabase側のテーブルとRLSは [docs/supabase/006_news_posts.sql](supabase/006_news_posts.sql) を適用してください。
- お知らせ画像用StorageバケットとRLSは [docs/supabase/007_news_storage.sql](supabase/007_news_storage.sql) を適用してください。

## エディタ補助

- エディタ下部の「文字列から盤面作成」では、`grid_strings` と互換の `0`〜`F` 複数行文字列を現在の盤面サイズへ反映できます。
- 小文字 `a`〜`f` は大文字へ正規化され、行数・文字数・使用文字が現在の盤面サイズと合わない場合は盤面へ反映しません。
- 入力欄は現在の盤面高さに合わせて表示行数を広げ、最大100行までの `grid_strings` 入力を扱いやすくしています。
- 入力・貼り付けできる文字は `0`〜`9` / `A`〜`F` / `a`〜`f` / 改行だけです。不正文字を含む貼り付けは拒否します。
- 「現在の盤面から文字列生成」で、作成中の盤面をAIや外部メモへ貼り付けやすい素の複数行文字列として出力できます。

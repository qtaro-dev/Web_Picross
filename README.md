...
Supabase Authでは正規のメールアドレス + パスワードで登録します。ログイン画面ではユーザー名とパスワードだけを入力し、アプリ側の `/api/resolve-login-email` が `profiles.username` からログイン用メールアドレスを解決してSupabase Authへ渡します。ユーザー登録、パスワード再設定、確認メール再送では引き続きメールアドレス入力が必要です。

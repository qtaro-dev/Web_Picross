...
async function requireUser(req){
  ...
  const { data:profile, error:profileError } = await client
    ...
    .maybeSingle();
  if(profileError) throw Object.assign(new Error('ユーザー情報を確認できません'), { status:403 });
  if(profile?.account_status === 'disabled') throw Object.assign(new Error('このアカウントは利用停止中です'), { status:403 });
  return { user:userData.user, profile, token, client, supabaseUrl, publishableKey };
}

async function guardedUser(req, res, handler){
  try{
